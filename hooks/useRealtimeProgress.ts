// 'use client';

// import { useEffect, useState, useCallback, useRef } from 'react';
// import { usePathname } from 'next/navigation';
// import { useUserProgress } from './useUserProgress'; // ⭐ IMPORTANT

// interface ProgressUpdate {
//   type: 'page' | 'quiz';
//   pageNumber?: number;
//   quizNumber?: number;
//   message: string;
// }

// interface UseRealtimeProgressOptions {
//   minTimeOnPage?: number;
//   onProgressUpdate?: (update: ProgressUpdate) => void;
// }

// export function useRealtimeProgress(options: UseRealtimeProgressOptions = {}) {
//   const { minTimeOnPage = 6000, onProgressUpdate } = options;
//   const pathname = usePathname();
  
//   // ✅ IMPORTANT: Importer le Context pour mettre à jour la UI en temps réel
//   const { 
//     isProfessorMode, 
//     togglePageCompletion, 
//     toggleQuizCompletion,
//   } = useUserProgress();
  
//   const [timeOnPage, setTimeOnPage] = useState(0);
//   const [isValidated, setIsValidated] = useState(false);
//   const [showCheckmark, setShowCheckmark] = useState(false);
  
//   const pageStartTimeRef = useRef<number>(0);
//   const timerRef = useRef<NodeJS.Timeout | null>(null);
//   const validationTimeoutRef = useRef<NodeJS.Timeout | null>(null);

//   // Fonction pour extraire les informations de la page actuelle
//   const getCurrentPageInfo = useCallback(() => {
//     const pathParts = pathname.split('/');
    
//     if (pathParts[1] === 'chapitres') {
//       const chapterNumber = parseInt(pathParts[2], 10);
      
//       if (pathParts[3] === 'quiz') {
//         return {
//           type: 'quiz' as const,
//           chapterNumber,
//           pageNumber: null
//         };
//       } else if (pathParts[3] === 'introduction' || pathParts[3] === 'video') {
//         return null; // Pas de validation pour ces pages
//       } else {
//         const pageNumber = parseInt(pathParts[3], 10);
//         return {
//           type: 'page' as const,
//           chapterNumber,
//           pageNumber
//         };
//       }
//     }
    
//     return null;
//   }, [pathname]);

//   // Fonction de validation automatique
//   const validateCurrentPage = useCallback(async () => {
//     const pageInfo = getCurrentPageInfo();
//     if (!pageInfo || isValidated || isProfessorMode) return false;

//     console.log('🎯 [REALTIME] ===== VALIDATION AUTOMATIQUE =====');
//     console.log('📍 [REALTIME] Page info:', pageInfo);
//     console.log('⏱️ [REALTIME] Temps passé:', timeOnPage, 'ms');

//     try {
//       if (pageInfo.type === 'page' && pageInfo.pageNumber !== null) {
//         // Exclure certaines pages
//         if (
//           pageInfo.pageNumber === 0 ||
//           pageInfo.pageNumber === 30 ||
//           pageInfo.chapterNumber === 0 ||
//           pageInfo.chapterNumber === 11
//         ) {
//           return false;
//         }

//         // ⭐ Appel du togglePageCompletion - met à jour le Context DIRECTEMENT
//         await togglePageCompletion(pageInfo.pageNumber);
        
//         console.log('✅ [REALTIME] Page validée:', pageInfo.pageNumber);
        
//         setIsValidated(true);
//         setShowCheckmark(true);
        
//         // Callback optionnel
//         if (onProgressUpdate) {
//           onProgressUpdate({
//             type: 'page',
//             pageNumber: pageInfo.pageNumber,
//             message: `Page ${pageInfo.pageNumber} complétée`
//           });
//         }

//         // Masquer la coche après 3 secondes
//         setTimeout(() => {
//           setShowCheckmark(false);
//         }, 3000);

//         return true;
//       } else if (pageInfo.type === 'quiz') {
//         if (pageInfo.chapterNumber === 11) return false;

//         // ⭐ Appel du toggleQuizCompletion - met à jour le Context DIRECTEMENT
//         await toggleQuizCompletion(pageInfo.chapterNumber);
        
//         console.log('✅ [REALTIME] Quiz validé:', pageInfo.chapterNumber);
        
//         setIsValidated(true);
//         setShowCheckmark(true);
        
//         // Callback optionnel
//         if (onProgressUpdate) {
//           onProgressUpdate({
//             type: 'quiz',
//             quizNumber: pageInfo.chapterNumber,
//             message: `Quiz du chapitre ${pageInfo.chapterNumber} complété`
//           });
//         }

//         // Masquer la coche après 3 secondes
//         setTimeout(() => {
//           setShowCheckmark(false);
//         }, 3000);

//         return true;
//       }
//     } catch (error) {
//       console.error('❌ [REALTIME] Erreur validation:', error);
//       return false;
//     }
    
//     return false;
//   }, [
//     getCurrentPageInfo, 
//     isValidated, 
//     timeOnPage, 
//     onProgressUpdate,
//     isProfessorMode,
//     togglePageCompletion,
//     toggleQuizCompletion
//   ]);

//   // Démarrer le timer quand on arrive sur une nouvelle page
//   useEffect(() => {
//     console.log('🔄 [REALTIME] Nouvelle page détectée:', pathname);
    
//     // Reset des états
//     pageStartTimeRef.current = Date.now();
//     setTimeOnPage(0);
//     setIsValidated(false);
//     setShowCheckmark(false);
    
//     // Nettoyer les anciens timers
//     if (timerRef.current) clearInterval(timerRef.current);
//     if (validationTimeoutRef.current) clearTimeout(validationTimeoutRef.current);
    
//     // Démarrer le compteur de temps
//     timerRef.current = setInterval(() => {
//       const elapsed = Date.now() - pageStartTimeRef.current;
//       setTimeOnPage(elapsed);
//     }, 100); // Mise à jour toutes les 100ms pour fluidité
    
//     // Programmer la validation automatique après 6 secondes
//     validationTimeoutRef.current = setTimeout(() => {
//       console.log('⏰ [REALTIME] 6 secondes écoulées - validation automatique');
//       validateCurrentPage();
//     }, minTimeOnPage);
    
//     console.log('⏰ [REALTIME] Timer démarré pour:', pathname);
    
//     // Nettoyage
//     return () => {
//       if (timerRef.current) clearInterval(timerRef.current);
//       if (validationTimeoutRef.current) clearTimeout(validationTimeoutRef.current);
//     };
//   }, [pathname, minTimeOnPage, validateCurrentPage]);

//   // Fonction pour valider manuellement si le temps est suffisant
//   const validateIfTimeElapsed = useCallback(async () => {
//     if (timeOnPage >= minTimeOnPage && !isValidated) {
//       return await validateCurrentPage();
//     }
//     return false;
//   }, [timeOnPage, minTimeOnPage, isValidated, validateCurrentPage]);

//   return {
//     timeOnPage,
//     isValidated,
//     showCheckmark,
//     canValidate: timeOnPage >= minTimeOnPage && !isValidated,
//     validateIfTimeElapsed,
//     currentPageInfo: getCurrentPageInfo()
//   };
// }