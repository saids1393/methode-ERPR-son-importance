...
const handleContactSubmit = async (e: React.FormEvent) => {
  ...
  // Fonction pour récupérer les devoirs envoyés
  const fetchHomeworkSends = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error('Erreur lors du chargement des devoirs');
      }
    } catch (error) {
  ...
  const getHomeworkStatuses = (): HomeworkStatus[] => {
    ...
    for (let chapterId = 1; chapterId <= 10; chapterId++) {
      ...
      if (sentHomework) {
        ...
        // Vérifier si le chapitre est complété pour déterminer si le devoir est "à venir"
       let isChapterCompleted: any;
isChapterCompleted = isChapterCompleted(chapterId); // toujours déconseillé à cause du même nom
        ...
        <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-8 mb-12">
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h3 className="text-2xl font-bold text-white">Mes Devoirs</h3>
                <p className="text-slate-400">Suivez vos devoirs envoyés et à venir</p>
              </div>
            </div>
            
            <div className="text-right">
              <div className="text-sm text-slate-400">Devoirs envoyés</div>
              <div className="text-xl font-bold text-green-400">
                {homeworkLoading ? '...' : homeworkSends.length}
              </div>
            </div>
          </div>
          ...
          {homeworkLoading ? (
            <div className="text-center py-8">
              <div className="animate-spin rounded-full h-8 w-8 border-t-2 border-b-2 border-orange-400 mx-auto mb-4"></div>
              <p className="text-slate-400">Chargement des devoirs...</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {homeworkStatuses.map((homework) => (
                <div 
                  key={homework.chapterId} 
                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-slate-400 text-xs">
                          {homework.status === 'sent' ? 'Envoyé' : 'À venir'}
                        </p>
                      </div>
                    </div>
                  ...
                  {homework.status === 'pending' && (
                    <div className="text-xs text-orange-300">
                      {isChapterCompleted(homework.chapterId) 
                        ? 'Devoir en cours de préparation...' 
          ...
          {!homeworkLoading && homeworkStatuses.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-slate-400">
                Les devoirs apparaîtront au fur et à mesure de votre progression.
              </p>
            </div>
          )}
        </div>
