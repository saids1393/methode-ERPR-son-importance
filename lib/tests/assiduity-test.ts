...
export class AssiduityTester {
  ...
  private async analyzeTrends(users: any[]) {
    ...
    const studyTimeTrend = recentAvgStudyTime > olderAvgStudyTime * 1.1 ? 'increasing' :
                          recentAvgStudyTime < olderAvgStudyTime * 0.9 ? 'decreasing' : 'stable';
    ...
    const progressTrend = recentAvgProgress > olderAvgProgress * 1.1 ? 'increasing' :
                         recentAvgProgress < olderAvgProgress * 0.9 ? 'decreasing' : 'stable';

    // Tendance d'activité utilisateur
    const userActivityTrend = recentUsers.length > olderUsers.length ? 'increasing' :
